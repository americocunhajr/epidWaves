% -----------------------------------------------------------------
%  Main_COVID19_RegressionMC_1st_wave_RJ.m
% -----------------------------------------------------------------
%  This program constructs a data-driven algebraic statistical 
%  model to describe the evolution of COVID-19 notification on 
...
DateEnd    = datenum('07-01-2020');
TrainStart = datenum('05-01-2020');
TrainEnd   = datenum('07-01-2020');
...
RawDataEnd   = 183;  % Jul 01, 2020

% training data start/end
TrainDataStart = 122;  % May 01, 2020
TrainDataEnd   = 183;  % Jul 01, 2020
...
K0   = Cmax;
r0   = RegressionExp(time(92:122),Data_I_raw(92:122));
tau0 = tImax;
...
% % model distribution
% I_Supp = 1:0.1:365;
% I_CDF  = MyFit_C(I_Supp)/trapz(I_Supp,MyFit_I(I_Supp));
%     
% % generate samples via inverse transform method
% I_Samp = interp1(I_CDF,I_Supp,rand(N_train,1),'linear','extrap');
...
% ..........................................................
figure(1)
%ymax = max(Cmax,max(MyFit_C_env(:,2)));
ymax = 15000;
...
fig2e = plot([tau_ast tau_ast],[0 0.5*ymax],'--k','LineWidth',2);
hold off
